...
public class ReceiverPfeiffer {
    
    private static final Logger logger = LoggerFactory.getLogger(ReceiverPfeiffer.class);
    ...
    public static Double execReadPressure() {
        ...
        if (isWindows) {
            logger.debug("SerialController execReadPressure command : " +  String.valueOf(command));
        ...
        try {
            ...
            logger.debug("SerialController execReadPressure getInputStream : " +  String.valueOf(process.getInputStream())); 
            ...
            logger.debug("SerialController execReadPressure getInputStream result : " +  String.valueOf(result)); 
            
            try {
                int exitCode = process.waitFor();
                logger.debug("SerialController execReadPressure exitCode : " +  String.valueOf(exitCode)); 
                if (exitCode == 0) {
                    Integer pressureIndex = result.indexOf("Pressure");
                    logger.debug("SerialController execReadPressure exitCode : " +  String.valueOf(pressureIndex)); 
                    if (pressureIndex != -1) {
    ...
    public static Double readPressure(String port, Integer addr) {
        ...
        if (serialPort != null) {
            ...
            // logger.debug("SerialController sendDataRequest command temp : " + String.valueOf(command) + "   " + String.valueOf(command.getBytes().length));

            Integer checksum = computeChecksum(command);
            // logger.debug("SerialController sendDataRequest checksum : " + String.valueOf(checksum));
            ...
            try {
                while (serialPort.bytesAvailable() == 0 && retryCount < PORT_RETRY_TIMES) {
                    logger.debug("SerialController serialPort sleep retryCount : " + retryCount);
                    retryCount++;
                    Thread.sleep(10);
                }
                ...
                if (serialPort.bytesAvailable() > 0) {
                    ...
                    // logger.debug("SerialController serialPort read : " + String.valueOf(read));
                    String strBuffer = new String(buffer, StandardCharsets.UTF_8);
                    // logger.debug("SerialController serialPort strBuffer : " + String.valueOf(strBuffer));
                    
                    serialPort.closePort();
                    
                    byte data[] = Arrays.copyOfRange(buffer, 10, buffer.length-4); //  r[10:-4]
                    String strData = new String(data, StandardCharsets.UTF_8);
                    // logger.debug("SerialController serialPort strData : " + String.valueOf(strData));
